...
App({
  onLaunch: function () {
    ...
    console.log('app.js: 小程序启动 - 官方规范版本')
  ...
  initBackendSelection() {
    try {
      ...
      if (isDev) {
        ...
        console.log('开发环境：优先使用SCF后端（官方规范）')
      } else {
        // 生产环境：优先微信云开发
        this.globalData.backendConfig = {
          currentBackend: null, // 延迟决定
  ...
  async switchBackend(backend) {
    ...
    try {
      ...
      wx.showToast({
        title: `已切换到${backend === 'scf' ? 'SCF(官方规范)' : '微信云开发'}`,
        icon: 'success',
        duration: 2000
      })
  ...
  async initApiService() {
    try {
      // 检查是否启用官方规范模式
      if (this.globalData.backendConfig.officialMode) {
        // 动态加载官方规范版API服务
        const ApiServiceOfficial = require('./utils/api-official.js')
        this.globalData.apiService = ApiServiceOfficial

        console.log('官方规范版API服务初始化完成')
      } else {
        // 降级到原版API服务
  ...
  async warmUpCloudFunction() {
    try {
      ...
      if (backend === 'scf') {
        // SCF预热 - 使用官方规范的健康检查
        await this.globalData.apiService._scfAdapter.healthCheck()
      } else {
        // 微信云开发预热
