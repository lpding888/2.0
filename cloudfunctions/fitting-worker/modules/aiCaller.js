...
class AICaller {
  constructor() {
    this.timeout = 56000 // 56秒超时，留4秒给云函数处理错误
    this.retryCount = 2
  ...
  async callGeminiOpenAICompatible(model, params) {
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('API请求超时(56秒)')), this.timeout)
      })
  ...
  parseGoogleGeminiResponse(responseData) {
    try {
      ...
      if (images.length === 0 && textResponse) {
        console.log('⚠️ 没有找到inline图片数据，但有文本响应')
        console.log('📝 文本响应内容:', textResponse.substring(0, 200))
      }
  ...
  prepareAPIParams(model, prompt, parameters, images = []) {
    ...
    // 从parameters中排除images字段，避免覆盖convertedImages
    const { images: _unused, ...cleanParameters } = parameters || {}

    return {
      prompt: String(prompt || ''),
      images: convertedImages,  // 这个images来自函数参数（已处理的图片）
      ...cleanParameters         // 展开其他参数，但不包含images
    }
  }
  ...
  /**
   * 解析API密钥 (支持环境变量 + 多密钥轮询)
   */
  parseApiKey(apiKey) {
    // 🎯 多密钥轮询池（8个密钥支持60-80并发用户）
    const API_KEY_POOL = [
    ...
    ].filter(Boolean)  // 过滤掉未配置的密钥

    // 如果有多个密钥，随机选择一个（负载均衡）
    if (API_KEY_POOL.length > 0) {
      const randomIndex = Math.floor(Math.random() * API_KEY_POOL.length)
      const selectedKey = API_KEY_POOL[randomIndex]
      console.log(`🔑 使用API密钥池 (${randomIndex + 1}/${API_KEY_POOL.length})`)
      return selectedKey
    }
