...
class AICaller {
  ...
  parseOpenAICompatibleResponse(content) {
    try {
      ...
      // 这里应该有图片生成逻辑，但OpenAI兼容格式通常返回文本
      // 如果是图片生成API，需要根据实际响应格式调整
      console.log('📄 获得文本响应:', message.substring(0, 100) + '...')

      return {
        success: true,
        data: {
          images: [], // OpenAI兼容格式的图片处理需要根据具体API调整
          text_response: message,
          usage: content.usage
        },
        message: 'OpenAI兼容API调用成功'
      }
            ...
            if (part.inline_data && part.inline_data.data) {
              const imageData = {
                ...
                metadata: {
                  ...
                  format: 'base64',
                  size_kb: Math.round((part.inline_data.data.length * 3) / 4 / 1024)
                }
              }
              images.push(imageData)
              console.log(`✅ 获得图片 (候选${candidateIndex + 1}, 部分${partIndex + 1}): ${imageData.metadata.mime_type}, ${imageData.metadata.size_kb}KB`)
            }
          })
        }
      })
